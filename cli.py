...
If you have an OpenWeatherMap API key, the `weather storeapi` command wil prompt
you for it.
"""
...
historical_handler = logging.FileHandler(
    filename=os.path.join(DATA_PATH, "history.log"), mode="a"
)
...
def get_api_response(ctx: click.core.Context, api: str, location: str) -> dict:
    ...
    if os.path.exists(datapath) and (time.time() - os.stat(datapath).st_mtime) <= 600:
...
def main(ctx, api_key, api_key_file, quiet, verbose):
    ...
    if ctx.invoked_subcommand not in ["log", "showdata"]:
        lastrun_handler = logging.FileHandler(
            filename=os.path.join(DATA_PATH, "weather.log"), mode="w"
        )
...
def howmuchrain(ctx, location):
    ...
    for day in sorted(data):
        print(
            f"{day:%a %m/%d} {data[day]:5.2f}mm", f"({data[day]*0.03937:0.02} inches)"
        )
